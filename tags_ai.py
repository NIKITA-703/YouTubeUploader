...
# Точное поведение меняется, но безопасно держать общий объём <= 450–480 символов.
...
def _clean_hashtag(tag: str) -> str:
    ...
    if not t.startswith("#"):
        t = "#" + t
    # убираем пробелы внутри хэштега
...
def _clean_seo_tag(tag: str) -> str:
    ...
    if not t:
        return ""
    # Уберём решётки и кавычки
    t = t.replace("#", "").replace('"', "").replace("'", "")
    # Сильно не режем — YouTube нормально переваривает пробелы в keyword-тегах
    t = _normalize_space(t)
    return t


def _cap_youtube_tags(tags: List[str], max_total_chars: int = YOUTUBE_TAGS_MAX_TOTAL_CHARS) -> List[str]:
    """Оставляет теги так, чтобы суммарная длина (с запятыми) была в лимите."""
    out = []
    total = 0
    for t in tags:
        # YouTube keywords обычно считаются как CSV-поле
...
def generate_youtube_tags(
    ...
    artists = _extract_artists_from_title(beat_title)
    ...
    # Схема ответа: строго и предсказуемо
    response_schema = {
        "type": "object",
        "properties": {
            "hashtags": {
    ...
    # Контент: даём модели контекст и правила
    # Важно: лучше просить на английском теги для type beat ниши — они обычно так и пишутся
    prompt = {
        "title": beat_title,
...
if __name__ == "__main__":
    # Впиши свой ключ сюда (как ты уже делал с genai.Client(api_key="..."))
    API_KEY = ""
